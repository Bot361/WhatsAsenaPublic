...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({pattern: 'meme ?(.*)', fromMe: false, desc: Lang.MEMES_DESC}, (async (message, match) => {    
	    ...
	    memeMaker({
            image: location,         
            outfile: 'Godzilla-meme.png',
            topText: topText,
            bottomText: bottomText,
        }, async function(err) {
            if(err) throw new Error(err)
            await message.client.sendMessage(message.jid, fs.readFileSync('Godzilla-meme.png'), MessageType.image, {filename: 'Godzilla-meme.png', mimetype: Mimetype.png, caption: 'Made by Punsith'});
            await info.delete();    
        });
    }));
}
